            ...
            <form action="{{ route('tasks.store') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea 
                        ...
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('description') }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <input type="radio" name="is_completed" value="1" id="completed" class="mr-2" 
                        {{ old('is_completed') == 1 ? 'checked' : '' }}>
                    <label for="completed" class="text-gray-700 dark:text-gray-200">Completed</label>

                    <input type="radio" name="is_completed" value="0" id="pending" class="ml-4 mr-2" 
                        {{ old('is_completed') == 0 ? 'checked' : '' }}>
                    <label for="pending" class="text-gray-700 dark:text-gray-200">Pending</label>
                    @if ($errors->has('is_completed'))
                        <span class="text-white text-sm">{{ $errors->first('is_completed') }}</span> 
                    @endif
                </div>
